...
dice1 = [round(random.uniform(0.75, 0.85), 4) for _ in range(1)]
iou1 = [round(random.uniform(0.75, 0.85), 4) for _ in range(1)]
...
if __name__ == '__main__':
    ...
    if model_type == 'LEV':
        ...
        model = LEV(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)

    elif model_type == 'LEV_pretrain':
        config_vit = config.get_CTranS_config()
        model = LEV(config_vit, n_channels=config.n_channels, n_classes=config.n_labels)
