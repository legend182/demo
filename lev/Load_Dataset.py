...
#         return len(os.listdir(self.output_path))
        return len(self.mask_list)  # 取两者最小值，避免越界
...
#         self.mask_list = os.listdir(self.output_path)
        self.images_list = [
            ...
            if not f.startswith('.') and f.endswith(('.jpg', '.png', '.jpeg'))
            ]
        self.mask_list = [
            f for f in os.listdir(self.output_path) 
            if not f.startswith('.') and f.endswith(('.png', '.gif'))
        ]
        self.one_hot_mask = one_hot_mask
        self.rowtext = row_text
        self.task_name = task_name
        self.bert_embedding = BertEmbedding()
        print(f"Number of images: {len(self.images_list)}")
        print(f"Number of masks: {len(self.mask_list)}")
        if joint_transform:
            self.joint_transform = joint_transform
        else:
            to_tensor = T.ToTensor()
...
#         return len(os.listdir(self.input_path))
        return min(len(self.images_list), len(self.mask_list))  # 取两者最小值，避免越界
...
#         image = cv2.resize(image, (self.image_size, self.image_size))
        if image is None:
            print(f"Warning: Failed to load {image_filename}, using placeholder.")
