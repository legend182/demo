...
from networks.LDCM import LDCM
...
#         dice1, dice2, dice3 = cal_dice(outputs,masks)
        dice1, dice2, dice3,hd95_et, hd95_tc, hd95_wt = cal_dice(outputs,masks)
...
#         
        hd95et_train+=hd95_et.item()
        hd95tc_train+=hd95_tc.item()
        hd95wt_train+=hd95_wt.item()
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
    ...
    hd95et_train = hd95et_train / len(train_loader)
    hd95tc_train = hd95tc_train / len(train_loader)
    hd95wt_train = hd95wt_train / len(train_loader)
#     return {'loss':loss,'dice1':dice1,'dice2':dice2,'dice3':dice3}
    return {'loss':loss,'dice1':dice1,'dice2':dice2,'dice3':dice3,'hd95_et':hd95et_train,'hd95_tc':hd95tc_train,'hd95_wt':hd95wt_train}
...
def val_loop(model,criterion,val_loader,device):
    ...
    with torch.no_grad():
        for images, masks in pbar:
            ...
            hd95et_val+=hd95_et.item()
            hd95tc_val+=hd95_tc.item()
            hd95wt_val+=hd95_wt.item()
    ...
    hd95et_val = hd95et_val / len(val_loader)
    hd95tc_val = hd95tc_val / len(val_loader)
    hd95wt_val = hd95wt_val / len(val_loader)
    
    return {'loss':loss,'dice1':dice1,'dice2':dice2,'dice3':dice3,'hd95_et':hd95et_val,'hd95_tc':hd95tc_val,'hd95_wt':hd95wt_val}
...
def train(model,optimizer,scheduler,criterion,train_loader,
    ...
    for e in range(epochs):
        ...
        info2 = "Train--ET: {:.3f} TC: {:.3f} WT: {:.3f} hd95_et:{:.3f} hd95_tc:{:.3f} hd95_wt:{:.3f}".format(train_metrics['dice1'],train_metrics['dice2'],train_metrics['dice3'],train_metrics['hd95_et'],train_metrics['hd95_tc'],train_metrics['hd95_wt'])
        info3 = "Valid--ET: {:.3f} TC: {:.3f} WT: {:.3f} hd95_et:{:.3f} hd95_tc:{:.3f} hd95_wt:{:.3f}".format(val_metrics['dice1'],val_metrics['dice2'],val_metrics['dice3'],val_metrics['hd95_et'],val_metrics['hd95_tc'],val_metrics['hd95_wt'])
        print(info1)
        print(info2)
        print(info3)
...
def main(args):
    ...
    model = LDCM(in_channels=4,num_classes=4).to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='./data/BraTS_data/')
    parser.add_argument('--train_txt', type=str, default='./data/train.txt')
    parser.add_argument('--valid_txt', type=str, default='./data/val.txt')
